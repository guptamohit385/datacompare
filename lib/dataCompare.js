...
function getprdiction(arr, list, accu, comparision, index){
    let results = []
    arr.forEach((i,r)=>{
        if(i > accu){
            results.push([r, i, list[r], comparision, index])
        }
    })
...
module.exports = function (listOfSen, sampleTest, accu, customShorthands, callback) {
    ...
    model.fit(xs, ys, trainConfig).then(() => {
        ...
        sampleTest.forEach((dta, index)=>{
            ...
            finalRespArr.push(getprdiction(arr, listOfSen, ((accu)? accu: 0.35), dta, index))
        })
